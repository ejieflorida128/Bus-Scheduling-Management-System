...
<body>
    <div class="frame">
        <div class="card">
          <?php    
                $id =  $_SESSION['id'];
                $sql = "SELECT * FROM passenger_account WHERE id = $id";
      ...
      <div class="card-body">
             <div class="row">
             <div class="forProfileimage">
                  <img src="<?php echo $check['profile_picture'] ?>" alt="no photo" class = "profile" id = "profile_image">
                  <img src="<?php echo $check['national_id'] ?>" alt="no id" class = "national_id" id = "national_id">
                </div>
             ...
             <div class="row">
                                ...
                                <div class="forBus">
                                    <label for="bus">Contact Number:</label>
                                    <input type="text" id = "bus" name = "bus" class = "form-control" value = '<?php echo $check['contact'] ?>'>
                                </div>
                        ...
                        <div class="row">
                            <div class="buttons" style = "position:absolute; top: 380px; left: 35px;">
                                <label class = "btn btn-primary" for = "changeProfile" style = " box-shadow: 0 4px 8px rgba(4, 4, 4, 1.1);">Change Profile</label>
                                <input type="file" id = "changeProfile"  onchange = "displaySelectedImage(event)" hidden value = "<?php echo $check['profile_picture'] ?>">
                                
                                <label class = "btn btn-primary" for = "changeNationalId" style = " box-shadow: 0 4px 8px rgba(4, 4, 4, 1.1);">Add National ID</label>
                                <input type="file" id = "changeNationalId"  onchange = "DisplaySelectedID(event)" hidden value = "<?php echo $check['national_id'] ?>">

                                <button class = "btn btn-success" style = "margin-top: -8px; box-shadow: 0 4px 8px rgba(4, 4, 4, 1.1);" onclick = "SaveAllDataFromTheCardProfile()" >Confirm Edit</button>
                                <div class="row">
...
<div class="modal" tabindex="-1" id = "SuccesfullDlete" style = "margin-top: 150px;">
  <div class="modal-dialog">
    ...
    function DisplaySelectedID(event) {
        ...
        reader.onload = function(event) {
            var imgElement = document.getElementById('national_id');
            imgElement.src = event.target.result;
        };
    ...
    function  SaveAllDataFromTheCardProfile(){
            ...
            var getNationalId = $('#changeNationalId').val();
            var ConfirmProfileEdit = true;
            
            var profile = getProfile.replace(/C:\\fakepath\\/i, '');
            var NationalId = getNationalId.replace(/C:\\fakepath\\/i, '');
